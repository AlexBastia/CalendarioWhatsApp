<script>
  ...
  export function show() { modalComponent?.show(); }
  export function hide() { modalComponent?.hide(); }
...
<Modal id="sharePomodoroModal" title="Condividi Pomodoro" bind:this={modalComponent}>
  ...
  <div class="list-group list-group-flush">
    {#if pomodoro.sharedUsers?.length === 0}
      <p class="text-muted">Nessuno. La condivisione viene aggiunta quando il destinatario accetta.</p>
    {/if}
    {#each pomodoro.sharedUsers ?? [] as user (user._id)}
      <form method="POST" action="?/removeUser" use:enhance={onEnhanceRemove} class="list-group-item d-flex justify-content-between align-items-center p-1">
