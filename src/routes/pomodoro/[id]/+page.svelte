...
async function runCycles(startCycle = 1) {
  ...
  for (let i = startCycle; i <= cicli; i++) {
    ...
    const result = await runPhase(studioSec, presetsPomodoro.fineStudio.title, presetsPomodoro.fineStudio.options);
    console.log('Fine studio', result);
    if (!running) break;
    

    if (i < cicli) {
      const gg  = await runPhase(pausaSec, presetsPomodoro.finePausa.title, presetsPomodoro.finePausa.options);
...
{:else}
  ...
  <div class="container min-vh-100 d-flex flex-column justify-content-center align-items-center">
    ...
    <div class="w-100 d-flex justify-content-center align-items-center mb-3 px-3" style="max-width: 900px;">
    ...
    <div class="row w-100 justify-content-center mb-3">
      <div class="col-10 col-sm-8 col-md-6 col-lg-6 d-flex justify-content-center">
        ...
        <svg class="w-100" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg">
          <circle cx="50" cy="50" r={r} fill="white" />
          <circle cx="50" cy="50" r={r} fill="none" stroke="gray" stroke-width="2" />
          <circle
            class="progress-ring"
            cx="50"
            cy="50"
    ...
    <div class="w-100 d-flex justify-content-center mb-3">
      <div class="d-flex gap-2 align-items-center justify-content-center">
        {#if !running}
          <BtnPom ariaLabel="Avvia Pomodoro" extraClasses="p-5" iconSrc={PlayIcon} on:click={() => { runCycles(); }} />
        {/if}

        {#if running}
          <BtnPom ariaLabel="Pausa/Riprendi" extraClasses="p-5" iconSrc={paused ? PlayIcon : PauseIcon} on:click={() => (paused = !paused)} />
          <BtnPom ariaLabel="Ferma Pomodoro" extraClasses="p-5" iconSrc={WhatsappIcon} on:click={stopTimer} />
          <BtnPom
